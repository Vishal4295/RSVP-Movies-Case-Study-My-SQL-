...
select 
      ...
      sum(case when date_published is null then 1 else 0 end) as date_pub_null_count , 
      ...
      sum(case when worlwide_gross_income is null then 1 else 0 end) as wgi_null_count , 
...
-- The 4 coulmns having null values are country, worlwide_gross_income, languages and production_company
...
-- Number of movies releases each month
...
with production_company_hit_movie_summary as (
                                              ...
                                              rank() over(
                                                          order by count(movie_id) desc) as production_company_rank
                                              from ratings as r
                                              inner join movie as m
                                              on m.id = r.movie_id
...
on r.movie_id = m.id
where avg_rating > 8
group by genre limit 3 ), movie_summary as (select genre, year, title as movie_name,
...
+-------------------+-------------------+---------------------+*/
-- Type your code below:
with production_company_summary as (select production_company, count(*) as movie_count
     from movie as m
     inner join ratings as r
     on r.movie_id = m.id
